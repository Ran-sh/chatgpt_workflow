...
9. Remove the active task only when `delete_active_task_on_completion` is true and completion has actually occurred.
10. Commit/push only paths allowed by `completion_commit_contract` and repository policy.

## 7. Result handoff

The Result Contract must identify the task/source revision, execution status, changed files, validation outcomes, blockers, and report path required by the project schema.
